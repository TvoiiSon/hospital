                    ...
                    <div class="mid-day">
                        <form action="vendor/push-appointment.php" method="post" class="timegrid">
                            ...
                            <?php
                                ...
                                $test = mysqli_query($link, "SELECT * FROM `timework` LEFT JOIN `appointment` ON `appointment`.`timeticket`=`timework`.`time` WHERE `appointment`.`iddoctor` = '$infdocid' ORDER BY `timework`.`id`");
                        ...
                        if(empty($test)) {
                            ...
                            <form action="vendor/push-appointment.php" method="post">
                            <input type="hidden" name="id" value='<?php echo $infdoc['id']; ?>'>
                            <?php foreach($timeticket as $tt) { ?>
                                <input type="submit" name="time" class="btn-save" value="<?php echo $tt[1]; ?>">
                            <?php } ?>
                            </form>
