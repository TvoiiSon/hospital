...
<head>
    ...
    <title>Мой Кабинет | Время Записи</title>
</head>
<body>
    <main>
        <header>
        ...
        <article>
            <div class="main-info" style="position: relative;">
                <p>Запись к врачу <strong>Фельдшер</strong> - Василий Пупкин Максимович</p>
                <div class="mi-date">
                    <div class="mid-day">
                        <form action="push/push-appoint-1.php" method="post" id="timeform" class="timegrid">
                            ...
                            <?php
                                ...
                                if($freedate = $datenow) {
                                    $test = mysqli_query($link, "SELECT * FROM `timework` LEFT JOIN `appointment` ON `appointment`.`timeticket`=`timework`.`time` WHERE `appointment`.`timeticket` IS NULL");
