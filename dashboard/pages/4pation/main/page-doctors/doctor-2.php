    ...
    <main>
        ...
        <article>
            <div class="main-info" style="position: relative;">
                <p>Запись к врачу <strong>Окушер</strong> - Алексей Буняев Кирилович</p>
                <div class="mi-date">
                    <div class="mid-day">
                        <form action="push/push-appoint-2.php" method="post" class="timegrid">
                            ...
                            <?php
                                ...
                                if($freedate = $datenow) {
                                    $test = mysqli_query($link, "SELECT * FROM `timework-2` LEFT JOIN `appointment-2` ON `appointment-2`.`timeticket`=`timework-2`.`time` WHERE `appointment-2`.`timeticket` IS NULL");
